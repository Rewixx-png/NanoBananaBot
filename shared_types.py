...
def _build_text_system_prompt(allow_web_directive: bool = True, is_owner: bool = False) -> str:
    ...
    return SYSTEM_PROMPT + owner_note + date_note + (
        ...
        'ВАЖНО ДЛЯ МАТЕМАТИКИ: формулы ОБЯЗАТЕЛЬНО оборачивай в $$ ... $$ (блочные) или $...$ (встроенные). Пример: $$ E = mc^2 $$ или энергия $E = mc^2$. НЕ пиши голый LaTeX без обёртки — он не отрендерится. '
        f'{web_rule}'
    )
