...
async def generate_text_with_gemini(prompt: str, chat_id: int, username: str='') -> str:
    ...
    for key in keys.copy():
        ...
        payload = {'systemInstruction': {'parts': [{'text': SYSTEM_PROMPT}]}, 'contents': contents, 'generationConfig': {'temperature': 1.0, 'thinkingConfig': {'thinkingBudget': 0}}}
...
async def generate_image_with_gemini(prompt: str, image_bytes: Optional[bytes]=None, model: str='gemini-3.1-flash-image-preview', images_bytes: list=None, temperature: float=1.0) -> Tuple[Optional[bytes], Optional[str]]:
    ...
    for key in keys.copy():
        ...
        for img in all_images:
            if img:
                parts.append({'inlineData': {'mimeType': 'image/jpeg', 'data': base64.b64encode(img).decode('utf-8')}})
        if len(all_images) > 1:
...
async def generate_code_with_gemini(prompt: str) -> str:
    ...
    for model_name in ['gemini-3.1-pro-preview', 'gemini-3.1-flash-preview']:
        for key in keys[:3]:
            ...
            gen_config = {'temperature': 0.4, 'maxOutputTokens': 65536, 'thinkingConfig': {'thinkingBudget': -1}}
...
async def fetch_openai_image_models() -> list:
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get('https://api.openai.com/v1/models', headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status == 200:
                    data = await resp.json()
...
_REPLICATE_MODELS = {'aisha-ai-official/wai-nsfw-illustrious-v12': {'version': '0fc0fa9885b284901a6f9c0b4d67701fd7647d157b88371427d63f8089ce140e', 'cfg_key': 'cfg_scale', 'input': lambda p: {'prompt': p, 'negative_prompt': 'lowres, bad anatomy, bad hands, text, error, missing fingers, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry', 'width': 896, 'height': 1152, 'steps': 28, 'cfg_scale': 7.0, 'scheduler': 'DPM++ 2M Karras'}}, 'aisha-ai-official/wai-nsfw-illustrious-v11': {'version': 'c1d5b02687df6081c7953c74bcc527858702e8c153c9382012ccc3906752d3ec', 'cfg_key': 'cfg_scale', 'input': lambda p: {'prompt': p, 'negative_prompt': 'lowres, bad anatomy, bad hands, text, error, worst quality, low quality', 'width': 896, 'height': 1152, 'steps': 28, 'cfg_scale': 7.0}}, 'aisha-ai-official/nsfw-flux-dev': {'version': 'fb4f086702d6a301ca32c170d926239324a7b7b2f0afc3d232a9c4be382dc3fa', 'cfg_key': 'guidance_scale', 'input': lambda p: {'prompt': p, 'width': 1024, 'height': 1024, 'steps': 28, 'guidance_scale': 3.5}}}

async def generate_image_with_replicate(prompt: str, model: str='aisha-ai-official/wai-nsfw-illustrious-v12') -> Tuple[Optional[bytes], Optional[str]]:
