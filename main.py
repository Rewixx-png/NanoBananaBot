    ...
    for task in pending:
        ...
        try:
            await bot.send_message(
                chat_id=chat_id,
                text=f"🔄 Бот был перезапущен. Продолжаю генерацию: {model_label}...",
            ...
            if task["gen_type"] == "video" and task.get("veo_operation_name"):
                ...
                if video_bytes:
                    await bot.send_video(
                        ...
                        caption=f"🎬 Видео ({model_label}) по запросу: {prompt}",
                        reply_to_message_id=source_msg_id,
                        **reply_kwargs
                    )
                else:
                ...
                if result:
                    await bot.send_photo(
                        ...
                        caption=f"🎨 Результат ({model_label}) после перезапуска: {prompt}",
                        reply_to_message_id=source_msg_id,
                        **reply_kwargs
                    )
                else:
